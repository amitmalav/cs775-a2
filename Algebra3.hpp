...
inline Vec2::Vec2(Vec3 const & v)  // caller should avoid divide-by-zero, the test here is not foolproof
{
  n[0] = v.n[0];
  n[1] = v.n[1];

  if (v.n[2] != 0)  // dangerous test!
  {
    n[0] /= v.n[2];
    n[1] /= v.n[2];
  }
...
inline Vec3::Vec3(Vec4 const & v)  // caller should avoid divide-by-zero, the test here is not foolproof
{
  n[0] = v.n[0];
  n[1] = v.n[1];
  n[2] = v.n[2];

  if (v.n[3] != 0)  // dangerous test!
  {
    n[0] /= v.n[3];
    n[1] /= v.n[3];
    n[2] /= v.n[3];
